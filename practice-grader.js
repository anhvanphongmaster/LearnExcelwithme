...
const ROLL_TOPICS=[["clean","🧹","Làm sạch dữ liệu"],["pq","⚙️","Power Query"],["input","⌨️","Nhập liệu"],["formula","ƒx","Công thức"],["format","🎨","Định dạng"]];

let currentTopic="clean",currentDifficulty="all",currentRankDifficulty="basic";
...
function renderTopicTabs(){
  const legacy=$("pgTopicTabs");
  if(legacy){legacy.innerHTML=TOPICS.map(([id,label])=>`<button type="button" class="pg-topic${id===currentTopic?" active":""}" data-topic="${id}">${label}</button>`).join("");}
  const stage=$("pgTopicRollStage");
  if(!stage)return;
  stage.innerHTML=ROLL_TOPICS.map(([id,icon,label],i)=>{
    const count=lessons().filter(l=>l.topic===id).length;
...
function renderLessons(){
  ...
  const cards=list.length?list.map(l=>card(l).replace('<article ','<article data-practice-roll-card ')).join(""):`<article class="pg-card pg-coming" data-practice-roll-card><h3>Không có bài phù hợp</h3></article>`;
  grid.innerHTML=`<div class="avp-practice-roll lesson-roll" id="pgLessonRoll" data-practice-roll data-roll-kind="lesson" tabindex="0"><div class="avp-roll-stage" data-practice-roll-stage>${cards}</div><div class="avp-roll-dots" data-practice-roll-dots></div></div><p class="avp-roll-hint">${list.length} bài trong chủ đề · kéo / vuốt / lăn để chuyển bài</p>`;
  ...
  const scope=grid;
  scope.querySelectorAll("[data-download]").forEach(b=>b.onclick=()=>{const l=lessons().find(x=>x.key===b.dataset.download);if(l)downloadLesson(l)});
  scope.querySelectorAll("[data-submit]").forEach(i=>{i.onchange=e=>{const f=e.target.files?.[0],lessonKey=i.dataset.submit,l=lessons().find(x=>x.key===lessonKey);if(l&&f)onFilePicked(l,f);e.target.value="";};});
  scope.querySelectorAll(".pg-file-submit").forEach(label=>{label.addEventListener("keydown",e=>{if((e.key==="Enter"||e.key===" ")&&!e.target.matches("input")){e.preventDefault();label.querySelector('input[type="file"]')?.click();}});label.setAttribute("tabindex","0");label.setAttribute("role","button");});
  scope.querySelectorAll("[data-review]").forEach(b=>b.onclick=()=>{const l=lessons().find(x=>x.key===b.dataset.review);if(l)showStoredReview(l)});
  scope.querySelectorAll("[data-guide-open]").forEach(b=>b.onclick=()=>window.AVPPracticeGuides?.open(b.dataset.guideOpen));
  scope.querySelectorAll("[data-visibility]").forEach(b=>b.onclick=()=>{const l=lessons().find(x=>x.key===b.dataset.visibility);if(l)toggleResultVisibility(l)});
  scope.querySelectorAll("[data-appeal]").forEach(b=>b.onclick=()=>{const l=lessons().find(x=>x.key===b.dataset.appeal);if(l)openAppeal(l)});
