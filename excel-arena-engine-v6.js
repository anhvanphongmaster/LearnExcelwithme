(()=>{
  ...
  const LEVELS=["Tân binh","Văn phòng","Analyst","Specialist","Pro","Master"];
  const MAX_DIFF=[1,2,3,4,5,5],LETTERS=["A","B","C","D"];
  const MIXED_MIN_LEVEL={"dynamic-array":2,"power-query":2,"vba":2,"workflow":2};
  ...
  const S={
    mode:"learn",topic:"mixed",running:false,player:"",current:null,options:[],correctIndex:0,
    used:new Set(),usedConcept:new Set(),recent:[],persist:[],retry:[],recentFamilies:[],recentTopics:[],
    seq:0,correct:0,wrong:0,score:0,combo:0,bestCombo:0,lives:3,level:1,cycle:1,
    raf:0,start:0,duration:18000,locked:false,hiddenAt:0,slotBag:[],lastSlot:-1,boardRows:[]
  };
  ...
  function eligible(c){
    if(!c||c.difficulty>maxDiff())return false;
    if(S.topic!=="mixed")return true;
    const min=MIXED_MIN_LEVEL[c.topic]||1;
    return S.level>=min;
  }
  function histKey(){return`avp_arena_s1_recent_${S.topic}`}
  function loadHist(){try{const x=JSON.parse(localStorage.getItem(histKey())||"[]");S.persist=Array.isArray(x)?x.filter(id=>conceptById.has(id)).slice(-80):[]}catch{S.persist=[]}}
  function remember(c){
    if(!c)return;
    const id=c.id,fam=familyKey(c);
    S.recent.push(id);if(S.recent.length>16)S.recent.shift();
    S.recentFamilies.push(fam);if(S.recentFamilies.length>6)S.recentFamilies.shift();
    S.recentTopics.push(c.topic);if(S.recentTopics.length>4)S.recentTopics.shift();
    S.persist=S.persist.filter(x=>x!==id);S.persist.push(id);S.persist=S.persist.slice(-80);
    try{localStorage.setItem(histKey(),JSON.stringify(S.persist))}catch{}
  }
  ...
  function chooseConcept(){
    ...
    if(!pool.length){
      S.cycle++;
      S.usedConcept.clear();
      pool=concepts().filter(eligible);
    }
    if(!pool.length)return null;

    let f=pool.filter(c=>!S.recent.includes(c.id));
    if(f.length>=2)pool=f;

    const old=new Set(S.persist.slice(-28));
    f=pool.filter(c=>!old.has(c.id));
    if(f.length>=2)pool=f;

    const recentFamilies=new Set(S.recentFamilies.slice(-4));
    f=pool.filter(c=>!recentFamilies.has(familyKey(c)));
    if(f.length>=2)pool=f;

    if(S.topic==="mixed"){
      const recentTopics=new Set(S.recentTopics.slice(-2));
      f=pool.filter(c=>!recentTopics.has(c.topic));
      if(f.length>=2)pool=f;
    }
  ...
  function variant(c,boss=false){
    let p=(byConcept.get(c.id)||[]).filter(q=>q.__arenaEnabled!==false&&!!q.boss===boss);
    if(!p.length)p=(byConcept.get(c.id)||[]).filter(q=>q.__arenaEnabled!==false&&!q.boss);
    if(!p.length)p=(byConcept.get(c.id)||[]).filter(q=>q.__arenaEnabled!==false);
    const u=p.filter(q=>!S.used.has(q.id));
    return shuffle(u.length?u:p)[0]||null;
  }
  ...
  function pick(){
    if(S.mode==="learn"){
      ...
      if(i>=0){
        const[x]=S.retry.splice(i,1),c=conceptById.get(x.id),q=c&&variant(c,false);
        if(q){S.used.add(q.id);remember(c);return{...q,isRetry:true}}
      }
    }
    const c=chooseConcept();
    if(!c)return null;
    const boss=S.seq>0&&(S.seq+1)%10===0,q=variant(c,boss);
    if(!q)return null;
    S.usedConcept.add(c.id);S.used.add(q.id);remember(c);
    return q;
  }
  ...
  function level(){return Math.min(6,1+Math.floor(S.correct/10))}
  function duration(q){const d=Math.max(1,Math.min(5,Number(q.difficulty)||1));return S.mode==="learn"?Math.max(15000,23000-d*1200):Math.max(9000,18500-(S.level-1)*700-d*600)}
  ...
  function makeOptions(q){
    ...
    let pool=(byTopic.get(q.topic)||[]).filter(c=>c.id!==q.conceptId&&Math.abs((c.difficulty||1)-(q.difficulty||1))<=1);
    if(pool.length<8)pool=(byTopic.get(q.topic)||[]).filter(c=>c.id!==q.conceptId);
    if(pool.length<3)pool=(B.concepts||[]).filter(c=>c.id!==q.conceptId);
    const distract=[];
    for(const c of shuffle(pool)){if(c.answer!==correct&&!distract.includes(c.answer))distract.push(c.answer);if(distract.length===3)break}
    while(distract.length<3)distract.push("Không có đáp án phù hợp");
    const slot=nextSlot(),opts=distract.slice(0,3);opts.splice(slot,0,correct);return{opts,slot}
  }
  ...
  function hintText(q){const m=meta(q.topic);if(S.mode==="learn")return `${q.meaning}. · Nhóm ${m.name}.`;return `Gợi ý: ${m.name} · mức ${q.difficulty}/5 · ${q.scenario||q.meaning}`}
  ...
  function render(q){if(!q)return end("Không còn câu phù hợp ở mức hiện tại.");S.current=q;S.locked=false;S.seq++;S.level=level();S.duration=duration(q);S.start=performance.now();$("questionDifficulty").textContent=`Độ khó ${q.difficulty}/5`;$("questionTopic").textContent=meta(q.topic).name;$("bossBadge").hidden=!q.boss;$("roundBadge").textContent=q.isRetry?"ÔN LẠI":`ROUND ${S.seq}`;$("questionPrompt").textContent=cleanPrompt(q.prompt);$("questionHint").querySelector("p").textContent=hintText(q);renderOptions(q);fb("",q.isRetry?"↻ Câu này đang được ôn lại.":q.boss?"⚠ Boss Round · điểm thưởng cao hơn.":"Chọn đáp án phù hợp nhất.");hud();updateRace();$("timerFill").style.width="100%";stopAnim();S.raf=requestAnimationFrame(anim)}
  function retry(q){if(S.mode!=="learn"||!q?.conceptId||S.retry.some(x=>x.id===q.conceptId))return;S.retry.push({id:q.conceptId,due:S.seq+4+Math.floor(Math.random()*5)})}
  ...
  function correct(index){S.locked=true;stopAnim();lockAnswers(index);const q=S.current;S.correct++;S.combo++;S.bestCombo=Math.max(S.bestCombo,S.combo);const p=points(q);S.score+=p;flashRace("good");fb("good",`<strong>CHÍNH XÁC +${p}</strong> · ${esc(q.answer)} — ${esc(q.meaning)}${S.combo>=3?` · Combo x${S.combo}`:""}`);hud();updateRace();later(q.boss?1300:1050)}
  function miss(index,reason){if(S.locked||!S.current)return;S.locked=true;stopAnim();lockAnswers(index);const q=S.current;S.wrong++;S.combo=0;retry(q);if(S.mode==="rank")S.lives--;flashRace("bad");fb("bad",`<strong>${reason==="timeout"?"HẾT GIỜ":"CHƯA ĐÚNG"}</strong> · Đáp án: ${esc(q.answer)} — ${esc(q.meaning)}${S.mode==="learn"?"<br>Câu này sẽ được ôn lại sau.":""}`);hud();updateRace();if(S.mode==="rank"&&S.lives<=0)setTimeout(()=>end("Hết 3 mạng."),1400);else later(1400)}
