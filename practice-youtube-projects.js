(()=>{
  ...
  function ensureHomeworkStyle(){if(document.getElementById('pytHomeworkStyle'))return;const s=document.createElement('style');s.id='pytHomeworkStyle';s.textContent='.pyt-homework{display:inline-flex;align-items:center;justify-content:center;margin-top:8px;min-height:39px;padding:0 12px;border:1px solid #e1c77d;border-radius:10px;background:#fff8e7;color:#765912!important;text-decoration:none;font-weight:900;font-size:12px}.pyt-homework:hover{background:#fff3cf}.dark-mode .pyt-homework{background:#332d1b;border-color:#65562f;color:#f1d987!important}';document.head.appendChild(s)}
  ...
  function partCard(p,i,project){
    const image=safeUrl(p.image_url),video=safeUrl(p.video_url),num=p.part_number||i+1,hw=p.homework||{},hasHomework=hw.enabled===true&&hw.status==='published';
    const hwKey=`${project.slug||project.id}-p${num}`;
    return `<article class="pyt-card ${tone(p.tone)}" data-practice-roll-card><div class="pyt-top"><span class="pyt-num">P${esc(num)}</span><span class="pyt-badge ${video?"ok":"soon"}">${video?"Có video":"Sắp có video"}</span></div><h2>${esc(p.title)}</h2><p>${esc(p.description||"")}</p>${image?`<figure class="pyt-card-media"><img src="${esc(image)}" alt="${esc(p.image_alt||`Minh hoạ ${p.title}`)}" loading="lazy"></figure>`:""}${video?`<a class="pyt-yt" href="${esc(video)}" target="_blank" rel="noopener">▶ Xem trên YouTube</a>`:'<span class="pyt-muted">Video: sắp gắn link</span>'}${hasHomework?`<a class="pyt-homework" href="homework.html?homework=${encodeURIComponent(hwKey)}">✍️ Làm bài tập của video này</a>`:''}</article>`;
  }
  ...
  function renderDetail(project){
    ...
    detail.innerHTML=`<button class="roll-back" type="button" id="youtubeProjectBack">← Quay lại Project Library</button><div class="avp-roll-head"><div><span>${esc(project.title).toUpperCase()}</span><h2>${parts.length} phần của project</h2></div><small>Phần ở giữa là phần đang chọn</small></div>${tools.length?`<div class="pyt-project-tools">${tools.map(r=>resourceLink(r,false)).join("")}</div>`:""}<div class="avp-roll-shell"><div class="avp-practice-roll pyt-phase-roll" data-practice-roll data-roll-kind="lesson" tabindex="0"><div class="avp-roll-stage" data-practice-roll-stage>${parts.map((part,i)=>partCard(part,i,project)).join("")}</div><div class="avp-roll-dots" data-practice-roll-dots></div></div><p class="avp-roll-hint">Kéo / vuốt · lăn chuột · phím ← →</p></div>${practice.length?`<div class="pyt-stage-file"><strong>File thực hành dùng chung</strong><p>${practice.map(r=>resourceLink(r,true)).join("")}</p></div>`:""}`;
