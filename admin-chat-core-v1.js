(function(){
  ...
  let latestUnreadPreviewInFlight=null;
  async function getLatestUnreadPreview(){
    if(latestUnreadPreviewInFlight)return latestUnreadPreviewInFlight;
    latestUnreadPreviewInFlight=(async()=>{
      ...
      try{
        ...
        if(isAdmin){
          ...
          const unreadThread=list.find(x=>Number(x.unread_count??x.unread??x.unread_messages??0)>0);
          if(!unreadThread)return null;
          const tid=String(unreadThread.thread_id??unreadThread.id??"");
          if(!tid)return null;
          const messages=await rpc("avp_chat_admin_thread_messages",{p_thread_id:tid})||[];
          const arr=Array.isArray(messages)?messages:[];
          const msg=[...arr].reverse().find(m=>m.sender_type==="user");
          if(!msg)return null;
          return {role:"user",thread_id:tid,sender:unreadThread.display_name||unreadThread.full_name||unreadThread.email||"Học viên",body:cleanChatPreviewText(msg.body),created_at:msg.created_at||null};
        }

        const session=await rpc("avp_chat_get_or_create_thread");
        const tid=String(session?.thread_id??session?.id??session??"");
        if(!tid)return null;
        const messages=await rpc("avp_chat_user_messages",{p_limit:150})||[];
        const arr=Array.isArray(messages)?messages:[];
        const msg=[...arr].reverse().find(m=>m.sender_type==="admin");
        if(!msg)return null;
        return {role:"admin",thread_id:tid,sender:"Anh Văn Phòng",body:cleanChatPreviewText(msg.body),created_at:msg.created_at||null};
      }catch(e){
        console.warn("AVP latest unread preview",e);
        return null;
      }
    })();
    try{return await latestUnreadPreviewInFlight}finally{latestUnreadPreviewInFlight=null}
  }
