/*! home-page-motion.js — V103 four-flow Home + expandable extras hub */
(function(){
  'use strict';
  if(window.__avpHomeMotionV103)return;
  ...
  function markMotion(){
    ...
    document.querySelectorAll('.home-path-card,.home-more-card,.home-book-card,.home-ref-item-v114,.home-cta-card,.learn-board,.home-platform-module-v1,.home-extra-head-v2').forEach(function(node){
  ...
  function wirePrimaryCta(){
    ...
    if(btn&&btn.dataset.learningHub!=='fourflows1'){
      var clean=btn.cloneNode(true);clean.dataset.learningHub='fourflows1';
      ...
      clean.addEventListener('click',function(e){e.preventDefault();location.href='skill-map.html';});btn.replaceWith(clean);
    }
    var learn=document.querySelector('.top-simple-nav [data-avp-nav="learn"]');
    if(learn){learn.href='skill-map.html';learn.setAttribute('aria-label','Chọn luồng học Excel');}
  }
  ...
  function renderExtras(shell){
    ...
    if(!head){
      head=document.createElement('div');head.className='home-extra-head-v2';
      head.innerHTML='<div><span>TIỆN ÍCH · KHU BỔ TRỢ</span><h3>Mở rộng kỹ năng</h3><p>Các khu nâng cao và công cụ dùng khi cần. Sau này thêm tính năng, bố cục sẽ tự chia lại mà không ép card quá nhỏ.</p></div><b>Khám phá thêm</b>';
      more.insertAdjacentElement('beforebegin',head);
    }
    more.className='home-path-more home-path-more--extras';
    more.innerHTML=''+
      '<div class="home-more-card home-extra-card-v2 tone-master is-restoring" aria-disabled="true" role="status"><span class="home-more-icon">◆</span><div><small>LỘ TRÌNH NÂNG CAO</small><strong>Excel Master</strong><em>Hệ nội dung chuyên sâu</em><b>Đang khôi phục</b></div></div>'+
      '<a class="home-more-card home-extra-card-v2 tone-skill" href="skill-map.html"><span class="home-more-icon">◈</span><div><small>BẢN ĐỒ KỸ NĂNG</small><strong>Skill Map</strong><em>Xem toàn bộ luồng và vị trí đang học</em><b>Mở bản đồ →</b></div></a>'+
      ...
      '<a class="home-more-card home-extra-card-v2 tone-arena home-more-race" href="excel-race.html"><span class="home-more-icon">⚡</span><div><small>GAME & PHẢN XẠ</small><strong>Excel Arena</strong><em>Đấu kỹ năng Excel theo thời gian</em><b>Vào Arena →</b></div></a>'+
    ...
    more.dataset.extrasVersion='2';
    return true;
  }

  function renderModules(){
    var shell=document.querySelector('.home-path-inner');var grid=shell&&shell.querySelector('.home-path-grid');if(!shell||!grid)return false;
    var title=shell.querySelector(':scope > h2');var desc=shell.querySelector(':scope > p');
    if(title)title.textContent='Chọn luồng học Excel';
    if(desc)desc.textContent='4 luồng · 42 bài. Mỗi luồng có màu và mục tiêu riêng; chọn một luồng rồi cuộn qua từng bài.';
    grid.className='home-path-grid home-platform-grid-v1 home-learning-flow-grid-v1';
    grid.innerHTML=tracks.map(function(t){return '<a class="home-platform-module-v1 home-learning-flow-v1 tone-'+esc(t[6])+'" href="skill-map.html?track='+encodeURIComponent(t[4])+'"><span class="home-platform-no-v1">'+esc(t[0])+'</span><span class="home-platform-copy-v1"><small>'+esc(t[1])+'</small><strong>'+esc(t[2])+'</strong><em>'+esc(t[3])+'</em><b>'+t[5]+' bài · Mở luồng →</b></span></a>';}).join('');
    grid.dataset.avpHomeAz='v103-fourflows';
    renderExtras(shell);wirePrimaryCta();document.body.classList.add('avp-home-ready');return true;
  }
  ...
  function boot(){renderModules();renderArena();ensurePracticeBanner();renderFirstRun(null);repairPodium();observeBoard();markMotion();loadCatalog();}
