(()=>{
  ...
  function bind(){
    ...
    $("aytParts")?.addEventListener("click",async e=>{const b=e.target.closest("[data-ayt-remove-part]");if(!b||!editing)return;const ok=await window.avpConfirm("Phần này sẽ được bỏ khỏi project sau khi bạn bấm Lưu.",{title:"Bỏ phần khỏi project?",tone:"warn",ok:"Bỏ phần",cancel:"Hủy"});if(!ok)return;editing.parts.splice(Number(b.dataset.aytRemovePart),1);renderParts()});
    $("aytResources")?.addEventListener("click",async e=>{const b=e.target.closest("[data-ayt-remove-resource]");if(!b||!editing)return;const ok=await window.avpConfirm("Tài nguyên này sẽ được bỏ khỏi project sau khi bạn bấm Lưu.",{title:"Bỏ tài nguyên?",tone:"warn",ok:"Bỏ tài nguyên",cancel:"Hủy"});if(!ok)return;editing.resources.splice(Number(b.dataset.aytRemoveResource),1);renderResources()});
    $("aytProjectList")?.addEventListener("click",async e=>{const b=e.target.closest("[data-ayt-act]"),row=b&&rows.find(x=>String(x.id)===b.closest("[data-ayt-id]")?.dataset.aytId);if(!b||!row)return;const act=b.dataset.aytAct;if(act==="edit")return openEditor(row);if(act==="toggle"){b.disabled=true;try{await rpc("admin_youtube_project_set_active",{p_id:row.id,p_active:!row.is_active});toast(row.is_active?"Đã ẩn project":"Đã hiện project");await load()}catch(err){toast("Không đổi được trạng thái");b.disabled=false}}if(act==="archive"){const ok=await window.avpConfirm("Project sẽ ẩn khỏi trang YouTube nhưng dữ liệu vẫn được giữ lại.",{title:"Lưu trữ project?",tone:"warn",ok:"Lưu trữ",cancel:"Hủy"});if(!ok)return;b.disabled=true;try{await rpc("admin_youtube_project_archive",{p_id:row.id});toast("Đã lưu trữ project");await load()}catch(err){toast("Không lưu trữ được");b.disabled=false}}});
