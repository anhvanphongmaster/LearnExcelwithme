...
function achStats(){const courses=achRead('completedCourses',[]);const pg=achRead(ACH_PG,{});const pgDone=Object.values(pg).filter(Boolean).length;const quiz=Math.min(5,parseInt(localStorage.getItem('quizBestScore')||'0',10)||0);const bonus=Number(localStorage.getItem('avp_bonus_xp_v1')||0)||0;const streak=achStreak(achRead(ACH_ACTIVITY,[]));const challenge=achRead('avp_excel_challenge_stats_v1',{});const challengeCorrect=Array.isArray(challenge.correctUnique)?new Set(challenge.correctUnique.map(Number)).size:0;const challengeScore=Number(challenge.score)||0;const challengeXP=challengeCorrect*5;const learningPathXP=window.getLearningPathXP?window.getLearningPathXP():0;const xp=Math.min(6,courses.length)*20+pgDone*10+quiz*10+bonus+challengeXP+learningPathXP;return{courses:Math.min(6,courses.length),pgDone,quiz,bonus,streak,challengeCorrect,challengeScore,challengeXP,learningPathXP,xp,level:Math.floor(xp/100)+1}}
