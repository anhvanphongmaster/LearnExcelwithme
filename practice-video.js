(() => {
  ...
  function renderTopics(){
    ...
    const accessibility=()=>{root.querySelectorAll('[data-practice-roll-card]').forEach(card=>{card.querySelector('button').tabIndex=card.classList.contains('is-roll-active')?0:-1});root.querySelectorAll('[data-practice-roll-dot]').forEach((dot,index)=>{dot.setAttribute('aria-label','Chủ đề '+topics[index].title);dot.setAttribute('aria-pressed',String(dot.classList.contains('active')))})};
    root.addEventListener('avp:roll-change',accessibility);window.AVPPracticeRoll?.initAll(mount);accessibility();
  }
  function lessonCard(row){
    const date=row.published_at?new Date(row.published_at).toLocaleDateString('vi-VN'):'';
    return `<article class="tt-lesson-card" id="tt-lesson-${T.esc(row.id)}"><div class="tt-lesson-meta"><span>VIDEO TIKTOK</span>${date?`<time datetime="${T.esc(row.published_at)}">${T.esc(date)}</time>`:''}</div><h3>${T.esc(row.title)}</h3>${row.description?`<p>${T.esc(row.description)}</p>`:''}<div class="tt-lesson-actions"><a class="tt-watch" href="${T.esc(T.videoUrl(row.tiktok_url))}" target="_blank" rel="noopener noreferrer">▶ Xem TikTok</a><button type="button" data-tt-download="${T.esc(row.id)}">↓ Tải file thực hành</button></div><p class="tt-file-caption">${T.esc(row.file_name||'File thực hành')} · Đăng nhập để tải</p><p class="tt-action-notice" data-tt-file-notice role="status" hidden></p></article>`;
  }
  function renderLessons(highlight=''){
    ...
    $('ttLessons').innerHTML=rows.length?rows.map(lessonCard).join(''):'<div class="tt-empty"><strong>Không tìm thấy bài phù hợp</strong><p>Thử tên bài ngắn hơn hoặc xóa từ khóa.</p><button type="button" data-tt-clear>Xóa tìm kiếm</button></div>';
    if(highlight){const card=$('tt-lesson-'+highlight);if(card){card.classList.add('tt-highlight');card.scrollIntoView({block:'center',behavior:'smooth'})}else showNotice('Bài trong liên kết không còn được phát hành. Bạn có thể chọn bài khác trong chủ đề này.')}
  }
