-- Professional Track: grader publish guard V1
-- Production migration name: professional_grader_publish_guard_v1
-- Applied 2026-09-09. This file mirrors the production guard; it does not contain hidden Case rubric details.
...
begin
  ...
  if source_path is not null and not exists (select 1 from storage.objects o where o.bucket_id='professional-track-resources' and o.name=source_path) then raise exception 'source_object_missing'; end if;
  if guide_path is not null and not exists (select 1 from storage.objects o where o.bucket_id='professional-track-resources' and o.name=guide_path) then raise exception 'guide_object_missing'; end if;
  ...
  if coalesce(p_published,false) then
    ...
    if coalesce(p_submission_enabled,true) then
      select o.updated_at into reference_updated_at from storage.objects o where o.bucket_id='professional-track-grading' and o.name=p_case_key || '/reference.xlsx' limit 1;
      if reference_updated_at is null then raise exception 'published_case_requires_reference'; end if;
      select o.updated_at into validation_updated_at from storage.objects o where o.bucket_id='professional-track-grading' and o.name=p_case_key || '/validation.json' limit 1;
      if validation_updated_at is null or validation_updated_at < reference_updated_at then raise exception 'grader_validation_required'; end if;
    end if;
  end if;
  ...
  on conflict(case_key) do update set
    title=excluded.title,goal=excluded.goal,tasks=excluded.tasks,skills=excluded.skills,
    expected_output=excluded.expected_output,duration=excluded.duration,max_score=10,
    source_url=excluded.source_url,guide_url=excluded.guide_url,
    submission_enabled=excluded.submission_enabled,published=excluded.published,
