-- Professional Track grader validation state V2
-- Replaces Storage validation.json with one JSONB state on professional_track_cases_v2.
-- This keeps the grading bucket XLSX-only and avoids an extra validation file/object.

alter table public.professional_track_cases_v2
  add column if not exists grader_validation jsonb;
...
begin
  ...
  if coalesce(p_published,false) then
    ...
    if coalesce(p_submission_enabled,true) then
      ...
      if validation_state is null or coalesce(validation_state->>'status','') <> 'validated' then
        raise exception 'grader_validation_required';
      end if;
      ...
      if validation_reference_updated_at is distinct from reference_updated_at then raise exception 'grader_validation_stale'; end if;
      if test_pass is distinct from 10::numeric or test_hardcode is null or test_hardcode >= 7 or test_wrong is null or test_wrong >= 7 then
